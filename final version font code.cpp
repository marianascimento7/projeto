...
enum DOS_COLORS{
         LIGHT_CYAN , AZUL , VERDE , CIANO , VERMELHO , MAGENTA , MARROM ,
        LIGHT_GRAY , DARK_GRAY , LIGHT_BLUE , LIGHT_GREEN , PRETO ,
...
DOS_COLORS CPrincipais [ 16 ] = {  LIGHT_CYAN , AZUL , VERDE , CIANO , VERMELHO , MAGENTA , MARROM , LIGHT_GRAY , DARK_GRAY ,
                        LIGHT_BLUE , LIGHT_GREEN ,PRETO , LIGHT_RED , LIGHT_MAGENTA ,AMARELO , BRANCO
						 };
...
void mVertical(int *aux,int max,char opc);
	...
	do{	
		...
		if(opc == ENTER){
			...
			switch(aux){
				...
				//Se for selecionada a segunda opção então então e chamada a função para ver as passagens
				case 1:
					lMoldura(C_MINIMA,C_MAXIMA,L_MINIMA,L_MAXIMA);
					check(d1(),x,y);
					menuP(0);
				...
				//Se for selecionada a qurta opção então a função para deslogar e chamada	
				case 3:
					lMoldura(C_MINIMA,C_MAXIMA,L_MINIMA,L_MAXIMA);
					dLogar();
					menuP(1);
...
int compare(zap jubiscleito, create ac){ //funcao para verificacao de login//
	...
	fread(&jubiscleito,1,sizeof(zap),fptr);
	

		for(i= 0 ;jubiscleito.key[i] != NULL;i++){
...
void ask(void){
	...
	gotoxy(coluna, 15);
	printf("E Necess\x85rio uma conta para reservar passagens\n");
	//Sleep(sleep);
...
//Recebe aux que destacara algumas das opções
//Uma condiçõo que representa a condição se imprimira o titulo ou não
void tabela1(int aux,int cond){
	int c;
	if(cond == 0){
		textcolor(PRETO);
...
//Função que imprime o menu do cadastramento de passagens
//Recebe aux que destacara algumas das opções
//Uma condiçõo que representa a condição se imprimira o titulo ou não
void tabela2(int cond,int aux){
	int c,l = 10;
...
//Função que imprime o menu do cadastramento de email
//Recebe aux que destacara algumas das opções
//Uma condiçõo que representa a condição se imprimira o titulo ou não
void tabela4(int cond,int aux){
	int c,l = 10;
...
//Função que imprime o menu de logar conta
//Recebe aux que destacara algumas das opções
//Uma condiçõo que representa a condição se imprimira o titulo ou não
void tabela6(int cond,int aux){
	int c,l = 10;
...
//Função para controlar as variaveis auxiliares nas outras funções
//Receber um ponteiro para variavel aux
//Um int que representa o numero maximo de opções
//E um char contendo a opçõa do teclado
void mVertical(int *aux,int max,char opc){
	//Se opção escolhida for para subir e aux ja estiver no numero minimo então aux recebera numero max
	if(*aux == 0 && opc == SETA_CIMA){
		*aux = max;		//Aux receber numero maximo
	}
	//Se opção escolhida for para descer e aux ja estiver no numero maximo então aux recebera numero minimo
	else if(*aux == max && opc == SETA_BAIXO){
		(*aux) = 0;//Aux recebe o numero minimo
		}
		//Se opção for para abaixar seta então aux sera incrementada
		else if(opc == SETA_BAIXO){
			(*aux)++;//Incrementação da aux
			}
			//Se opção for para seta para cima então aux sera decrementada
			else if(opc == SETA_CIMA)
				(*aux)--;	//Decremento da aux
				
}

//Função que controlara as coordenadas das setas da função menuP
//Recebe um ponteiro que corresponde a posição da seta
//Um int que representa o numero maximo de opçoes
//Dois vetores int que guardam as localizações das linhas e colunas das seta
//E um char que reprenta a opção escolhida
void setaM(int *aux,int max,int coluna[],int linha[],char opc){
	//Numero maximo decrementado para 1 ficar = 0 e assim po diante
	max --;
	
	//Se opção escolhida for para subir e aux ja estiver no numero minimo então aux recebera numero max
	if(*aux == 0 && opc == SETA_CIMA){
		(*aux) = max;					//Aux receber numero maximo
		aSeta(coluna[0],linha[0]);		//Apaga a seta anterior
	//	seta(coluna[*aux],linha[*aux]);	//Alocação da nova posição da seta
	}
		//Se opção escolhida for para descer e aux ja estiver no numero maximo então aux recebera numero minimo
		else if((*aux) == max && opc == SETA_BAIXO){
			(*aux) = 0;						//Aux recebe o numero minimo
			aSeta(coluna[max],linha[max]);	//Apaga a seta anterior
			seta(coluna[*aux],linha[*aux]);	//Alocação da nova posição da seta
			}
			//Se opção for para abaixar seta então aux sera incrementada
			else if(opc == SETA_BAIXO){
				(*aux)++;	//Incrementação da aux
				aSeta(coluna[*aux-1],linha[*aux-1]);//Apaga a seta anterior
				seta(coluna[*aux],linha[*aux]);	//Alocação da nova posição da seta
			}
			//Se opção for para seta para cima então aux sera decrementada
			else if(opc == SETA_CIMA){
